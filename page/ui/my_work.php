...
if (
    ...
    (
        in_array($_SESSION['code_p'], ['1064046037'], true)
        || $_SESSION['code_p'] === '1002'
    )
) {
    $special_view = true;
}
...
if (isset($_POST['user_name']) && $_POST['user_name'] !== '') {
    $user_name = str_p('user_name');
} elseif (isset($_GET['user_name']) && $_GET['user_name'] !== '') {
    $user_name = str_g('user_name');
}
if ($user_name === '0' || $user_name === 'all') $user_name = '';

$users_lookup = [];
$Query_users = "SELECT code_p, name FROM karbar";
if ($Result_users = mysqli_query($Link, $Query_users)) {
    while ($u = mysqli_fetch_array($Result_users)) {
        $users_lookup[$u['code_p']] = $u['name'];
    }
}
?>
                ...
                <form method="post" action="?page=s_new_karkerd" enctype="multipart/form-data" id="workForm">
                    <div class="row gx-3">
                        ...
                        <div class="col-lg-3 col-md-6 mb-2">
                            ...
                            <select class="form-select" name="faal" id="work_faal">
                        ...
                        <div class="col-lg-3 col-md-6 mb-2">
                            ...
                            <select class="form-select" name="daste" id="work_daste">
                                <?php
                        ...
                        <div class="col-lg-3 col-md-6 mb-2">
                            ...
                            <select class="form-select" name="sherkat" id="work_sherkat">
                                <?php
...
<?php if (in_array($_SESSION['code_p'], ['1064046037'], true) || $_SESSION['code_p'] === "1002") {
?>
    <div class="row gx-3">
        <div class="col-12">
            <div class="modern-card">
                ...
                <div class="modern-card-body">
                    <form method="post" action="?page=my_work" id="ticketFilterForm">
                        <div class="row gx-3">
                            ...
                            <div class="col-lg-3 col-md-6">
                                <div class="mb-3">
                                    <label class="form-label" for="user_name">مرتبط با کاربر</label>
                                    <select class="form-select" name="user_name" id="user_name">
                                        <?php
                                        $Query_user_name = "SELECT * from karbar where kind = 'software' OR code_p in ('$code_p_run') ORDER BY i_karbar DESC LIMIT 200";
                                        if ($Result_user_name = mysqli_query($Link, $Query_user_name)) {
                                            while ($q_user_name = mysqli_fetch_array($Result_user_name)) {
                                        ?>
                                                <option value="<?php echo $q_user_name['code_p']; ?>" <?php if ($user_name === $q_user_name['code_p']) echo "selected";
                                                                                                        ?>>
                                                    <?php echo $q_user_name['name']; ?>
                                                </option>
                                        <?php }
                                        } ?>
                                        <option value="" <?php if ($user_name === "" || $user_name === "0" || $user_name === "all") {
                                                                echo "selected";
                                                            } ?>>همه موارد</option>
                                    </select>
                                </div>
                            ...
                            <div class="col-lg-3 col-md-6 mb-2">
                                ...
                                <select class="form-select" name="faal" id="filter_faal">
                                    <option value="y" selected>کاری</option>
                                    <option value="a">آموزش</option>
                                    <option value="j">جلسات</option>
                                    <option value="z">تایم آزاد</option>
                        ...
                        <div class="d-flex justify-content-end gap-2 mt-3 pt-3 border-top"
                            ...
                            <button type="button" class="btn-outline-modern"
                                onclick="window.location.href='?page=my_work'">
                ...
                <div class="table-responsive">
                    <table class="table table-hover work-table m-0">
                        ...
                        <tbody>
                            ...
                            $Query_list = "SELECT * FROM karkerd WHERE 1=1";

                            /* اگر special view ندارد → فقط رکوردهای خودش */
                            if (!$special_view) {
                                $code_p_safe = mysqli_real_escape_string($Link, $code_p_run);
                                $Query_list .= " AND code_p = '$code_p_safe'";
                            }

                            /* اگر special view دارد و فیلتر کاربر انتخاب شده */
                            if ($special_view && !empty($user_name) && $user_name !== 'all' && $user_name !== '0') {
                                $user_name_safe = mysqli_real_escape_string($Link, $user_name);
                                $Query_list .= " AND code_p = '$user_name_safe'";
                            }

                            /* فیلتر تاریخ */
                            if (!empty($tarikh_1)) {
                                $tarikh_1_safe = mysqli_real_escape_string($Link, $tarikh_1);
                                $Query_list .= " AND tarikh_s >= '$tarikh_1_safe'";
                            }

                            if (!empty($tarikh_2)) {
                                $tarikh_2_safe = mysqli_real_escape_string($Link, $tarikh_2);
                                $Query_list .= " AND tarikh_s <= '$tarikh_2_safe'";
                            }

                            if (!empty($faal)) {
                                $faal_safe = mysqli_real_escape_string($Link, $faal);
                                $Query_list .= " AND faal = '$faal_safe'";
                            }

                            $Query_list .= " ORDER BY i_karkerd DESC LIMIT 1000";
                            ...
                            ?>
                                    <tr>
                                        ...
                                        <td>
                                            <?php
                                            $row_user_code = $q_kkk['code_p'];
                                            echo htmlspecialchars($users_lookup[$row_user_code] ?? 'نامشخص');
                                            ?>
                                        </td>
                                        <td><?php
...
<script>
(function() {
    var SEARCHABLE_SELECT_IDS = ['work_faal', 'work_daste', 'work_sherkat', 'filter_user_name', 'filter_faal'];
