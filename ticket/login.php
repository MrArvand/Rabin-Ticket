...
<head>
  ...
  <title>سامانه تیکت پشتیبانی فراهم - نسخه ویژه کاربران</title>
...
<body>
    ...
    <div class="auth-container">
      <div class="d-flex justify-content-center">
        ...
        <form action="page/ux/checklogin.php" method="post" class="login-card">
          ...
          <!-- Title -->
          <h1 class="login-title">سامانه ثبت درخواست</h1>
          <p class="login-subtitle">گروه صنعتی رهباریان</p>

          <!-- Info text -->
          <div class="info-text">
            کد پرسنلی و کلمه عبور ثبت شده در پالودیار را وارد نمایید
          </div>
  ...
  <script>
    ...
    document.querySelector('form').addEventListener('submit', function(e) {
      ...
      if (username === '' || password === '') {
        ...
        if (!errorDiv) {
          ...
          document.querySelector('.info-text').insertAdjacentElement('afterend', errorDiv);
        }
    ...
    document.querySelectorAll('.form-group, .d-grid, .version-badge, .login-title, .login-subtitle, .info-text').forEach(el => {
      observer.observe(el);
    });
    ...
    setTimeout(() => {
      document.querySelectorAll('.form-group').forEach((el, index) => {
        if (el.style.opacity === '0' || el.style.opacity === '') {
          el.style.animation = `fadeInLeft 0.6s ease-out ${1.2 + (index * 0.2)}s forwards`;
        }
      });
    }, 100);
